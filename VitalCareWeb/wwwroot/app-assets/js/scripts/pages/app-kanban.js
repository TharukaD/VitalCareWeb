...
$(document).ready(function () {
  ...
  var kanban_curr_el, kanban_curr_item_id, kanban_item_title, kanban_data, kanban_item, kanban_users;
    ...
    $.ajax({
          method: "POST",
          dataType: "json",
          url: "/projects/kanbanBoardData?id=" + $("#CurrentProjectId").val(),
      })
      .done(function( result ) {
          loanBoards(result);
      });

    function loanBoards(project) {

        console.log(project);
        
         // Kanban Board and Item Data passed by json
  var kanban_board_data = project.kanbanBoards;
  ...
  for (kanban_data in kanban_board_data) {
    ...
    for (kanban_item in kanban_board_data[kanban_data].item) {
      ...
      if (typeof $(board_item_el).attr("data-image") !== "undefined") {
        board_item_image =
              '<div class="kanban-image " style="width: 24px; padding-bottom: 7px;padding - top: 3px;">' +
