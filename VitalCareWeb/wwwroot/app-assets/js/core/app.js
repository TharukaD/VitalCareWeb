...
(function (window, document, $) {
  ...
  $(window).on('load', function () {
    ...
    // Filter
      ...
      $(".search-bar-results .input").typeahead(
          {
              minLength: 3,
              highlight: true,
              classNames: {
                  dataset: "search-list",
                  suggestion: "auto-suggestion d-flex align-items-center justify-content-between cursor-pointer p-1 text-secondary"
              }
          },
          {
              name: "loanContracts",
              ...
              source: function (query, syncResults, asyncResults) {
                  return $.getJSON("/search/loanContracts",
                      { query: query },
                      function (data) {
              ...
              templates: {
                  suggestion: function (data) {
                      var compiled =
                          _.template(
                              "<a href='<%= url %>'><div class='d-flex justify-content-start'><span class='pr-1 <%= icon %>'></span> <span class='search-result-name'><%= item %> </span></div></a>");
          ...
          },
          {
              ...
              limit: 5,
              source: function (query, syncResults, asyncResults) {
                  return $.getJSON("/search/customers",
                      { query: query },
                      function (data) {
                          return asyncResults(data);
                      });
          ...
          },
          {
              ...
              display: "item",
              source: function (query, syncResults, asyncResults) {
                  return $.getJSON("/search/properties",
                      { query: query },
                      function (data) {
                          return asyncResults(data);
                      });
          ...
          },
          {
              ...
              display: "item",
              source: function (query, syncResults, asyncResults) {
                  return $.getJSON("/search/transactions",
                      { query: query },
                      function (data) {
                          return asyncResults(data);
                      });
          ...
          },
          {
              ...
              display: "item",
              source: function (query, syncResults, asyncResults) {
                  return $.getJSON("/search/leaseContracts",
                      { query: query },
                      function (data) {
                          return asyncResults(data);
                      });
      ...
      $("search-bar-results .input").bind("typeahead:select",
          function (ev, suggestion) {
              location.href = suggestion.url;
          }
      );

      $(".search-bar-results .input").bind("typeahead:active",
          function (ev, suggestion) {
              $(".app-content").addClass("show-overlay")
          }
      );

      $(".search-bar-results .input").bind("typeahead:close",
          function (ev, suggestion) {
              if ($(".app-content").hasClass("show-overlay")) {
                  $(".app-content").removeClass("show-overlay")
              }
